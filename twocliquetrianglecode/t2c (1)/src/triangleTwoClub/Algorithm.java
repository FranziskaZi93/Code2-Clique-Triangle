...
public class Algorithm implements ITriangleTwoClubAlgorithm {
    ...
    public RtcGraph run(RtcGraph inputGraph, String graphName, String outputFile, boolean printInformation){
      ...
      //  chooseTnrOrLcrRule();
        ...
        for(int i = 0; i < startPoints.size(); i++){
            ...
            searchAtStartPoint(startPoints.get(i));
        }
    ...
    private void searchAtStartPoint(int startPointID){
        ...
        T2CGraph globalGraph = graph; // save global graph for later
    ...
    private void branch(int depth){
        ...
        if(useEdgeCondition ? conditionChecker.isEdgeLTriangleTwoClub(inputGraph) : conditionChecker.isLTriangleTwoClub(inputGraph)){
        ...
        int v = conditionChecker.getMostIncompatibleVertexID(inputGraph);
     ...
     */
    private boolean isStartPointValid(int startPointID){
     ...
     //       dataReductionRules.add(new EdgeTriangleRule(this));
       ...
       /**    DataReductionRule d = new LowCompatibilityRule(this);
        ...
        if(conflictGraphType != T2CGraph.ConflictGraphType.DISABLED)
            dataReductionRules.add(new MaximumMatchingRule(this)); // this rule requires a conflict graph jav v*/
    ...
    */
     ...
     */
    private void doInitialDataReduction(){
        for(DataReductionRule d : dataReductionRules)
            if(d.applyInitially())
                d.apply();
    }
