...
public class ConditionChecker {
    ...
    public ConditionChecker(ITriangleTwoClubAlgorithm algorithm){
    ...
    public boolean verticesAreCompatible( int v, int w, RtcGraph inputGraph){ //Knotensindpassend
        if(v == w)
            return true; //dann sind die Knoten ja gleich

        boolean compatible;
        if(algorithm.getConflictGraph() != null)//conflict graph bedingung auf disabled in algorithms klasse erledigt und kommentiert, kommt nur einmal vor, wenn ich es richtig gesehen habe
            compatible = !algorithm.getConflictGraph().adjacent(v, w); // wenn im Conflict graph nicht benachbart in wirklichkeit benachbart
        else
            compatible =  verticesAreCompatible(inputGraph, v, w); //checken ob compatible wann in else fall: hier nachgucken im Originalgraph, anders aufrufen

        if(!algorithm.isEdgeConditionEnabled() || !compatible)
            return compatible; // ungleich und ungleich ist gleich

        // Edge L>1 addition:
        // vw \in E => v, w incompatible if |N(w)\cap N(v)| < \ell
        // vw \notin E => there must be vertex u with |N(w)\cap N(u)|\ge \ell  and |N(u)\cap N(v)|\ge \ell
        int l = algorithm.getL();
        if(inputGraph.adjacent(v, w)){ //ursprünglicher Graph
            return countCommonNeighbors(v, w, l, inputGraph) == l; //
        }
        for(int u : inputGraph.getNeighbors(v)){ //welchen graph brauch ich bei get graph immer wieder überlegen // ursprünglicher graph, ich geh alle nachbarn von v durch, zäühle sie in u
            if(!inputGraph.getNeighbors(w).contains(u)) continue; // ursprünglicher graph muss hier genommen werden weil clique und nicht club
            if(countCommonNeighbors(v, u, l, inputGraph) == l && countCommonNeighbors(w, u, l, inputGraph) == l){
                return true;
            }
        }
   ...
   public static boolean Knotenpassend(RtcGraph inputGraph, int v, int w) {
       if (v == w) return true;
       if (inputGraph == null) return false;

       if (inputGraph.getNeighbors(v).contains(w))
           return true; //alle Knoten betrachten um Nachbarn zu findne, Orignalgraph behalten damit check durchführen

       HashSet<Integer> small;
       int toFind;
       if (inputGraph.getNeighbors(v).size() < inputGraph.getNeighbors(w).size()) {
           small = inputGraph.getNeighbors(v);
           toFind = w;
       } else {
           small = inputGraph.getNeighbors(w);
           toFind = v;
       }
       return small.stream().anyMatch(u -> inputGraph.getNeighbors(u).contains(toFind)); // hier wird mit Originalgraph gearbeitet
    ...
    public int zählepassendes(int v, int max, boolean countV, RtcGraph inputGraph){
        if(algorithm.getConflictGraph() != null){
            return algorithm.getConflictGraph().size() - algorithm.getConflictGraph().getNeighbors(v).size();
        }
        else
            return countCompatibles(inputGraph, v, max, countV); //ursprünglicher graph, und neuen, wieviele aktuelle knoten sind kompatibel (dafür origialgraph)
    }
    ...
    public int countCommonNeighbors(int v, int w, int max, RtcGraph inputGraph){
        RtcGraph g = inputGraph; //ursprünglicher graph
        if(!g.contains(v) || !g.contains(w) || max <= 0)
            return 0;
        if(v == w)
            return g.getNeighbors(v).size();
    ...
    public int zähleüblichenachbarn(int v, int w, int max, RtcGraph inputGraph){
        RtcGraph g = inputGraph; //ursprünglicher graph
        if(!g.contains(v) || !g.contains(w) || max <= 0)
            return 0;
        if(v == w)
  ...
  /**  public boolean isLTriangleTwoClub(){
    ...
    public boolean isLTriangleTwoClub(RtcGraph inputGraph){ //ist LDreickechzweiclub
        RtcGraph graph = algorithm.getGraph(); // durch den algorithmus bearbeitet
        int l = algorithm.getL();
        ...
        for(int v : graph.getVertices()){
            // is the distance between v and every other vertex at most two
                       for(int w : graph.getVertices())
                if(v < w && !verticesAreCompatible(inputGraph,v, w)) //hier wird inputgraph benutzt
                    return false;
        }
...
*/
    public boolean isEdgeLTriangleTwoClub (RtcGraph inputGraph){  //istkanteldreieckzweiclub    //ignorieren wenn nur vertex gemacht wird // ich könnte eventuell wieder den englischen namen verwenden
        int l = algorithm.getL();
        RtcGraph graph = algorithm.getGraph(); //durch den algorithmus bearbeitet
        ...
        for(Integer v : graph.getVertices()) {
            ...
            // check if every vertex is compatible to every other vertex
            for(Integer w : graph.getVertices())
                if(v < w && !verticesAreCompatible(inputGraph, v, w))
                    return false;
        }
        return true;
    }
...
**/
    public int countIncompatibles(int v, int currentWorst, RtcGraph inputGraph){ //zähleunpassendes
        if(!algorithm.getGraph().contains(v))
        ...
        for(int w : algorithm.getGraph().getVertices()){ //anpassen
            ...
            n--;
            if(!verticesAreCompatible(v, w, inputGraph))
                c++;
            if (c+n<= currentWorst){
                return 0;
            }
  ...
  /**  public int countIncompatibles(int v){
    ...
    public int zähleunpassendes(int v, RtcGraph inputGraph){
        if(!algorithm.getGraph().contains(v))
            return 0;
  ...
  /** public int getMostIncompatibleVertexID(){
   ...
   public int getMostIncompatibleVertexID(RtcGraph inputGraph){  //beschaffemeistenichtpasendevertexID
        int worstID = -1;
        int worstIncompatibilityCount = 0;
    ...
    private Integer findeW(HashSet<Integer> set1, HashSet<Integer> set2, RtcGraph inputGraph){
        if(set1 == null || set2 == null)
            return null;
        ...
        for(Integer w : set1){
            count = 0;
            for(Integer u : set2){
                if(verticesAreCompatible(inputGraph, w, u)) { //ursprünglich
                    // if there are enough compatible vertices for w, then try another vertex
                    if (++count >= algorithm.getL())
                        continue wLoop;
                }
